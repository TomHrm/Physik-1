...
\begin{document}
	...
	In einem Intertialsystem gilt \textbf{$F= m * a$} in seiner reinsten Form. Es ist damit ein Bezugssystem, in welchem sich ein kräftefreier Körper gradlinig gleichförmig bewegt. Die \textbf{Newton$^\prime$ schen Axiome} gelten damit \textbf{nur in Intertialsystemen}\\
	\section{Gravitation}
	...
	\section{Träge und schwere Masse}	
	...
	$S^2 = 4^2 + 3^2$
	\section{Reibung}
	\[ F_R = \mu \cdot F \qquad \vec{F}_\sqcup \text{i st entgegen } \vec{v} \]
	\begin{center}
	...
	$ F = - kx$ \\
	\[ \left[ W = \int \vec{F} d\vec{r} = \int_{x_0}^{0} F dx = -k \frac{x^2}{2} x_0  = \frac{1}{2} k x^2 \right] \]
	...
	\end{align*}
	\section{4.9 Potentielle Energie und Potential}
	...
	\begin{proof}
		...
		\[= m = \frac{ds}{dt}dv \]
		\[ = m \cdot v dv\]
		\[W = \int_{A}^{B} \vec{F} d\vec{r} = \int_{V_A}^{V_B} m \cdot v dv = \frac{1}{2}mv^2 \]
		\[  = \frac{1}{2} m ( \vec{v}_B^2 - \vec{v}_A^2 ) \]
		\[  E_{kin} = \frac{1}{2} m \vec{v}^2 = \frac{\vec{p}^2}{2m} \quad W = E_{kin, B} E_{kin, A}\]
	...
	\part{Vorlesung 12 - Übernahme durch Prof. Hagner}
	\begin{center}
		\includegraphics[scale=0.3]{IMG_A27E53A8E9C0-1.jpeg}
	\end{center}
	\textbf{Schwerpunkt: }
	
	
	\[  \vec{R}_S = \frac{m_1 \vec{r}_1 + m_2 \vec{r}_2 }{m_1 + m_2} \]
	Der Schwerpunkt ist also der Punkt wo das Objekt im Gleichgewicht liegt.
	\[ \vec{r}_{12} = \vec{r}_1 - \vec{r}_2  \] 
	\[ \vec{V}_S = \frac{m_1 \vec{v}_1 + m_2 \vec{v}_2 }{m_1 + m_2} \]
	
	\begin{figure}[htbp]
		\begin{minipage}[t]{7cm}
			\vspace{0pt}
			\centering
			\includegraphics[scale=0.20]{IMG_5C671F23ECB9-1.jpeg}
		\end{minipage}
		\hfill
		\begin{minipage}[t]{7cm}
			\vspace{0pt}
			\begin{align*}
			R_S = \frac{m_s \cdot0 + M_M \cdot R_{EM}}{m_E + M_M}\\
			R_S = \frac{m_M}{M_E + m_m} \cdot r_{EM} = 4600km\\
			m_M = 7 \cdot 10^{22} kg \quad \frac{m_M}{m_E} \approx 0.012\\
			m_E = 6 \cdot 6 \cdot 10 ^{24} kg\\
			r_{EM} = 384.400 km
			\end{align*}
		\end{minipage}
	\end{figure}
	
	\textbf{reduzierte Masse:}\\
	\[ \mu = \frac{m_1 \cdot m_2}{m_1 + m_2} \]
	Damit gilt: 
	\[ \vec{p} = m_1 \vec{v} + m_2 \vec{v}_2 = M \cdot \vec{v}_s \]
	\[ \frac{d}{dt} \vec{p} = \vec{F}_1 + \vec{F}_2 = \sum \vec{F} \]
	welches dann für die äußere Kraft steht.\\
	\subsection{kinetische Energie}
		Nun wollen wir die gesamte kinetische Energie bestimmen
		\[ E_{kin} = \frac{1}{2} m_1 v_1^2 + \frac{1}{2} m_2 v_2^2 \]
		\[ = \frac{1}{2} m_1 ( \vec{v}_S + \frac{m_2}{m_1 + m_2}  \vec{v}_{12})^2 + \frac{1}{2} m_2 (\vec{v}_s  \frac{m_1}{m_1 + m_2} \vec{v}_{12})^2 \]
		dabei haben wir genutzt dass:
		\[ \vec{v}_1 = \vec{v}_S + \frac{m_2}{m_1 + m_2} \vec{v}_{12} \quad \vec{v}_2  = \vec{v}_S + \frac{m_1}{m_1 + m_2} (- \vec{v}_{12} \]
		nun können wir noch weiter umformen
		\[ = \frac{1}{2} (m_1 + m_2) \vec{v}_S^2 + \frac{1}{2} ( \frac{m_1 m_2^2}{(m_1 + m_2)^2} + \frac{m_2 m_1^2}{(m_1 + m_2)^2}) v_{12}^2 \]
		Dabei heben sich die Terme $\vec{v}_S \vec{v}_{12}$ auf
		\[ E_{kin} = \underbrace{\frac{1}{2} Mv_S^2}_{\text{kin. Energie des SP}} + \underbrace{\frac{1}{2} \mu v^_{12}^2}_{\text{reduzierte  Masse,Relativgeschwindigkeit}} \]
		
		\textbf{Der Gesamt-Drehimpuls bezüglich des Koordianten Ursprungs}
		\[ \vec{L} = \vec{L}_1 + \vec{L}_2 = m \vec{r}_1 \times \vec{v}_1 + m_2 \vec{r}_2  \times \vec{v}_2 \]
		Auf dem Übungsblatt 6 gibt es dazu eine Aufgabe wo das genau ausgerechnet werden soll wobei das Ergebnis dann so aussieht:
		\[ \vec{L} = M \vec{R}_S \times \vec{v}_S + \mu \vec{r}_{12} \times \vec{v}_{12} \]
		\textbf{Gesamt. Drehmoment} (bzgl. Koordinaten Ursprung)
		\[ \vec{M} = \vec{M}_1 + \vec{M}_2 = \vec{r}_1 \times ( \vec{F}_{12} + \vec{F}_1) + \vec{r}_2  \times (-\vec{F}_{12} + \vec{F}_2) \]
		\[ = (/\vec{r}_1 - \vec{r}_2) \times \vec{F}_{12} + \vec{r}_1 \times \vec{F}_1  + \vec{r}_2 \times \vec{F}_2] \]
		Durch Überlegen wie sich die Skalarprodukte verhalten wird klar, dass folgendes herauskommen muss:
		
		\[ \vec{M} = \vec{r}_1 \times \vec{F}_1 + \vec{r}_2 \times \vec{F}_2  = \sum_{äußere} \vec{M} \]
		\[ \frac{d\vec{L}}{dt} = \frac{d \vec{L}_1}{dt} + \frac{ d\vec{L}_2}{dt} = \vec{M}_1 + \vec{M}_2 = \vec{M} = \sum_{äußere} \vec{M} \]
		Das ist nun also die \textbf{Drehimpulserhaltung} wenn keine äußere Kräfte wirken
		\subsection{sonderfall: Abgeschlossene Systeme}
		keine äußeren Kraft $\vec{F}_1 = \vec{F}_2 = 0$\\
		\[ \text{Impuls } \vec{P} = M \cdot \vec{V}_S = konst \]
		Schwerpunkt bewegt sich mit konstanter Geschwindigkeit
		\[ \text{Drehimpuls } \vec{L} = M \cdot \vec{R}_S \times \vec{a}_s + \mu \vec{r}_{12} \times \vec{v}_{12} = konst \]
		Drehimpuls ist erhalten
		\subsection{Das Schwerpunktsystem}
		\textbf{Def:}\\
		Koordinatenursprung liegt im Schwerpunkt
		\begin{figure}[htbp]
			\begin{minipage}[t]{7cm}
				\vspace{0pt}
				\centering
				\includegraphics[scale=0.20]{IMG_34D6AFF7119B-1.jpeg}
			\end{minipage}
			\hfill
			\begin{minipage}[t]{7cm}
				\vspace{0pt}
				\begin{align*}
				\vec{r}^\prime = \vec{r} - \vec{R}_S\\
				\vec{v}^\prime = \vec{v} - \vec{v}_S
				\end{align*}
			\end{minipage}
		\end{figure}
		obere äußere Kräfte:
		\[ \vec{v}_S = konst \Rightarrow \text{ CMS ist Inertialsyste} \]
		\[ \vec{R}_s^\prime = 0 \quad \vec{r}_{12}^\prime = \vec{r}_1^\prime - \vec{r}_2^\prime = \vec{r}_1 - \vec{r}_2 = \vec{r}_{12} \]
		\[ \vec{v}_S ^\prime = 0 \quad   \vec{v}_{12}^\prime = \vec{v}_1^\prime - \vec{v}_2^\prime = \vec{v}_1 - \vec{v}_2 = \vec{v}_{12\]
			Dabei ist $\vec{p}^\prime = 0$ und der Gesammtimpuls im SPS damit 0
			\[ m_1 v_1^\prime + m_2 \vec{v}_2^\prime  = 0\]
			\[ E^\prime_{kin} = \frac{1}{2}\mu \vec{v_{12}^2} \qquad \vec{L}^\prime = \mu \vec{r}_{12}^\prime \times \vec{v}_{12}^\prime \text{ also $L^\prime$ = konst} \]
			Die \textbf{Bewegunbgsgleichung} für $m_1, m_2$ reduziert sich auf eine Bewegungsgleichung der Polarkorrdinate
			\[ \frac{d}{dt} \vec{v}_{12}^\prime = \frac{d}{dt} \vec{v}_1^\prime - \frac{d}{dt} \vec{v}^\prime_2 = \frac{\vec{F}_{12}}{m_1} - \frac{\vec{F}_{12}}m_2} = \frac{\vec{F}_{12}}{m_1} + \frac{\vec{F}_{12}}{m_2}\] 
		\[ = (\frac{1}{m_1} + \frac{1}{m_2}) \vec{F}_{12} = \frac{1}{\mu} \vec{F}_{12} \]
		\textbf{Nun zum Ergebnis}
		\[ \mu \frac{d}{dt} \vec{v}_{12}^\prime = \vec{F}_{12} = \mu \frac{d^2 \vec{r}_{12}^\prime  }{dt^2} \]
		Als kurze zusammenfassung lässt sich sagen, dass wenn es keine äußeren Kräfte gibt sich das Zweikörperproblem reduzieren lässt auf ein Einkörperproblem mit reduzierter Masse $(\mu)$ und Relativkoordiante $( \vec{r}_{12})$
		\part{Vorlesung 13}
		\section{Stoßprozesse}
			\includepdf[pages=-]{./images/190528-Hag.pdf}
		\part{Verlesung 14
			\section{Dynamik starrer Körper}
			\includepdf[pages=-]{./images/190604-Hag.pdf}
		\part{Vorlesung 15}
			\textbf{Wiederholung:}\\
			\textbf{\begin{tabular}{|c|c|}
					\hline 
					Translation & Rotation (Um feste Achse) \\ 
					\hline 
					Länge x und Geschwindigkeit $\vec{v}$ & Winkel $\varphi$ \\ 
					\hline 
					Masse m & Trägheitsmoment $I = \int r^2 dm$ \\ 
					\hline 
					Impuls $\vec{p} = m \cdot \vec{v} $& Drehimpuls $\vec{L} = I \cdot \vec{w}$ \\ 
					\hline 
					Kraft $\vec{F} = \frac{d\vec{p}}{dt}$ & Drehmoment $\vec{D} = \vec{r} \times \vec{F} = \frac{d\vec{L}}{dt}$ \\ 
					\hline 
					$E_{kin} = \frac{1}{2} mv^2$ & $E_{rot} = \frac{1}{2} i w^2$
					\hline 
			\end{tabular} }
		Dabei entsprechen die Winkelgeschwindigkeit und die Geschwindigkeit sowie die Masse und das Trägheitsmoment dem gleichen Prinzip in verschiedenen Systemen
	\end{document}
