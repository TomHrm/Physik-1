...
\textbf{Verallgemeinerung}:\
	...
	\[  f_x_y = \frac{\partial}{\partial y} (\frac{\partial f}{\partial x}) = \frac{\partial^2 f}{\partial_y \partial_x} \]
		...
		\section{Vektoranalysis: Gradient, Divergenz, Rotation}
			...
			Ein \textbf{Shaderfeld} hat nur eine (skalare) Komponente:
				\[ f(\vec{r}) = f(x,y,z) \]\\
				\textbf{Gradient:}\\
				
				\[ f \rightarrow \vec{\nabla} f = grad f = \vec{\nabla} f = \left( \frac{\partial f}{\partial x}, \frac{\partial f}{\partial y}, \frac{\partial f}{\partial z} \right) \text{\qquad} Shaderfeld \rightarrow Vektorfeld \] 
				...
				Hierbei muss extrem auf die Vorzeichen aufgepasst werden. Es gibt zudem nur 2 Kombinationen wie man im laufe sehen wird die sich nicht wegkürzen.\\
					rot($\vec{F} (\vec{r}) = \vec{\nabla} \times \vec{F} = \sum_{i, j, k}^{}$ $\frac{\partial f_j}{\partial x_i}  \vec{e}{_i,j} = (\frac{\partial f_z}{\partial y} - \frac{\partial f_y}{\partial z}, \frac{\partial f_x}{\partial z} - \frac{\partial f_z}{\partial x}, \frac{\partial f_z}{\partial x} - \frac{\partial f_x}{\partial y}$)\\
