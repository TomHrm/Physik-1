				...
				Hierbei muss extrem auf die Vorzeichen aufgepasst werden. Es gibt zudem nur 2 Kombinationen wie man im laufe sehen wird die sich nicht wegkürzen.\\
					...
					\section{Grundlagen der Dynamik}
						...
						\end{itemize}
							...
							\subsubsection{Linien- und Flächenelemente:}
					...
					\subsection{Kugelkoordinaten - sphärische Polarkoordinaten}
						...
						\[ z = r cos \theta \]
					\subsection{rotierendes Bezugssystem und Scheinkräfte}
						\begin{center}
						...
						\end{center}
							...
							$\Rightarrow \frac{d \vec{b}}{dt} = \sum_{i}\frac{db_i}{dt} \underbrace{\vec{e}_i}_{Zeitunabhängig} = \sum_{i} \frac{dbi^\prime}{dt} \vec{e}_i^\prime = \sum_{b_i}^\prime \underbrace{\frac{d\vec{e}_i^\prime}{dt}}_{ \vec{w} \times \vec{e}_i^\prime} = \underbrace{\left( \frac{d \vec{b}}{dt} \right)^\prime}_{Ableitung im \sum^\prime-System} + \vec{w} \times \vec{b} $ \\
						\textbf{Ferner:}\\
						...
						\subsection{Spezielle Relativitätstheorie - Lorentstransformation}
							Galilei-Transformationen in der Newton´schen Mechanik fürhren zur Vektoraddition von Geschwindigkeiten: 
					...
					$ (c_0t^\prime)^2 - x^\prime^2 = c_0^2(cx + dt)^2 - \gamma^2(x -vt)^2 \\
						...
						\textbf{Vierervektor-Formatlismus:} \\
							...
							Spezialfall: Teilchen in Ruge im $\sum$, aber $\vec{p} = 0, E = m_0 = \text{\textit{Rugemasse}}$\\
							 ...
							 in alltäglichen Koordinaten: $E^2 = c_0^2 \vec{p}^2 + m_^2 c_0^4 \qquad [E] = [mv^2], [p]= [mv]$\\
							...
							\subsection{Allgemeine Form der Newtonschen Bewegungsgleichung:}
								...
								\subsubsection{Mehrteilchensysteme für N Teilchen}
									...
									\textbf{Siehe Dokument: Differentialgleichung.pdf} 
						...
						\subsubsection{Energie-Erhaltung}
							...
							\textbf{Taylor Entwicklung von K(h) um k = 0:}\\
								...
								$\Rightarrow \dot{\vec{L}} = m \dot\underbrace{{\vec{r}} \times \dot{\vec{r}}}_{= 0} + m \vec{ r} \times \ddot{\vec{r}} = \vec{r} \times \dot{\vec{p}} = \vec{ r} \times \vec{ F}$\\
